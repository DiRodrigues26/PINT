...
function gerarCsvAreas(items) {
  const linhas = [
    ['Nome da Area', 'Learning Path', 'Service Line', 'Data de Criacao', 'Estado'],
    ...items.map((area) => [
      area.nome,
...
function imprimirTabelaAreas(items) {
  const linhas = items.map((area) => `
    <tr>
      <td>${escaparHtml(area.nome)}</td>
      <td>${escaparHtml(area.nome_learning_path)}</td>
      <td>${escaparHtml(area.nome_service_line)}</td>
      <td>${escaparHtml(formatarData(area.created_at))}</td>
      <td>${area.ativo ? 'Ativo' : 'Inativo'}</td>
    </tr>
  `).join('');
    ...
    <html>
      ...
      <body>
        ...
        <table>
          <thead>
            <tr>
              <th>Nome da Área</th><th>Learning Path</th><th>Service Line</th>
...
export default function AdminAreas() {
  ...
  async function exportarExcel() {
    try {
      ...
      descarregarCsv('areas.csv', gerarCsvAreas(todos));
    } catch (err) {
