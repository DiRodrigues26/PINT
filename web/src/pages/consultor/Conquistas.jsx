...
const CORES_CONQUISTA = [
  { border: 'border-softinsa-400', bg: 'bg-softinsa-100', icon: 'text-softinsa-600', data: 'text-softinsa-600' },
  { border: 'border-blue-400',     bg: 'bg-blue-100',     icon: 'text-blue-600',     data: 'text-blue-600' },
  { border: 'border-amber-400',    bg: 'bg-amber-100',    icon: 'text-amber-600',    data: 'text-amber-600' },
  { border: 'border-violet-400',   bg: 'bg-violet-100',   icon: 'text-violet-600',   data: 'text-violet-600' },
  { border: 'border-emerald-400',  bg: 'bg-emerald-100',  icon: 'text-emerald-600',  data: 'text-emerald-600' },
];
...
function ItemProgresso({ conquista, index }) {
  ...
  const objetivo = Number(conquista.valor_objetivo) || 1;
  const pct = Math.min(Math.round((atual / objetivo) * 100), 100);

  let labelProgresso = `${atual} / ${objetivo}`;
  let unidade = '';
  ...
  else if (conquista.tipo_criterio === 'NIVEL') unidade = '';
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className={`text-xs font-semibold ${cor.data}`}>
              {labelProgresso}{unidade ? ` ${unidade}` : ''}
            </span>
          </div>
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
            <div className={`h-full rounded-full transition-all ${cor.border.replace('border-', 'bg-')}`} style={{ width: `${pct}%` }} />
          </div>
        </div>
      </div>
    </div>
...
export default function Conquistas() {
  ...
  const emProgresso = progressoData?.dados ?? [];
