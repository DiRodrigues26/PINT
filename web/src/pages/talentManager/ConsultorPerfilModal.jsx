...
const ESTADO_OBJ = {
  PENDENTE:  { label: 'Pendente',  cls: 'bg-slate-100 text-slate-600', icon: Circle },
  EM_CURSO:  { label: 'Em curso',  cls: 'bg-blue-100 text-blue-700',   icon: Clock },
  CONCLUIDO: { label: 'Concluído', cls: 'bg-emerald-100 text-emerald-700', icon: CheckCircle2 },
  CANCELADO: { label: 'Cancelado', cls: 'bg-rose-100 text-rose-600',   icon: X },
};
...
export default function ConsultorPerfilModal({ consultor, onFechar }) {
  ...
  const criarObjetivo = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Objetivo adicionado à timeline do consultor.');
  ...
  function submeterObjetivo() {
    if (!titulo.trim()) { toast.error('O objetivo precisa de um título.'); return; }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" onClick={onFechar}>
      <div className="relative flex max-h-[90vh] w-full max-w-xl flex-col rounded-2xl bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <h2 className="text-lg font-bold text-slate-900">Perfil do Consultor</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="flex items-center gap-4 rounded-xl bg-gradient-to-r from-[#eaf2fc] to-[#f4f9ff] p-5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-slate-500">Consultor{consultor?.nome_area ? ` · ${consultor.nome_area}` : ''}</p>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <div className="rounded-xl bg-softinsa-50 p-4 text-center">
              ...
              <p className="mt-0.5 text-xs text-slate-500">Badges Obtidos</p>
            </div>
            <div className="rounded-xl bg-amber-50 p-4 text-center">
              <p className="text-2xl font-bold text-amber-500">{Number(consultor?.pontos_totais ?? 0).toLocaleString('pt-PT')}</p>
              <p className="mt-0.5 text-xs text-slate-500">Pontos Totais</p>
            </div>
          ...
          <div className="mt-5">
            <h4 className="text-sm font-bold text-slate-900">Badges Conquistados</h4>
            {isLoading ? (
              ...
              <p className="mt-3 text-sm text-slate-400">Este consultor ainda não obteve badges.</p>
            ) : (
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between">
              <h4 className="flex items-center gap-2 text-sm font-bold text-slate-900"><Target className="h-4 w-4 text-softinsa-600" /> Timeline de Evolução</h4>
              {!mostrarForm && (
            ...
            {mostrarForm && (
              <div className="mt-3 space-y-2 rounded-xl border border-slate-200 p-4">
                <input value={titulo} onChange={e => setTitulo(e.target.value)} placeholder="Objetivo (ex.: Concluir certificação AWS)"
                ...
                <div className="flex items-center gap-2">
                  <input type="date" value={dataFim} onChange={e => setDataFim(e.target.value)} title="Prazo"
                  ...
                  <button type="button" onClick={submeterObjetivo} disabled={criarObjetivo.isPending}
                    className="rounded-lg bg-softinsa-600 px-4 py-2 text-sm font-semibold text-white hover:bg-softinsa-700 disabled:opacity-60">Adicionar</button>
                  <button type="button" onClick={() => setMostrarForm(false)} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancelar</button>
                </div>
              </div>
            )}

            {timeline.length === 0 ? (
              <p className="mt-3 text-sm text-slate-400">Sem objetivos definidos para este consultor.</p>
            ) : (
              <div className="mt-3 space-y-2">
                {timeline.map(o => {
                  ...
                  return (
                    <div key={o.id_objetivo} className="flex items-start gap-3 rounded-xl border border-slate-200 p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {o.data_fim && <p className="mt-0.5 text-[11px] text-slate-400">Prazo: {formatarData(o.data_fim)}</p>}
                      </div>
                      <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${est.cls}`}>{est.label}</span>
                    </div>
                  );
                })}
              </div>
