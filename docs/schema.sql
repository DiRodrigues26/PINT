-- Schema exportado de softinsa_badges
-- Gerado para visualização de diagrama ER
...
CREATE TABLE `device_token` (
  ...
  `plataforma` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id_token`),
  UNIQUE KEY `uq_device_token` (`token`),
  KEY `idx_dt_utilizador` (`id_utilizador`),
  CONSTRAINT `fk_dt_utilizador` FOREIGN KEY (`id_utilizador`) REFERENCES `utilizador` (`id_utilizador`) ON DELETE CASCADE
...
DROP TABLE IF EXISTS `politica_rgpd`;
CREATE TABLE `politica_rgpd` (
  `id_politica` int unsigned NOT NULL AUTO_INCREMENT,
  `tipo_politica` varchar(80) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'GERAL',
